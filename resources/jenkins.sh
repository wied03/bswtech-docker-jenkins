...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
  ...
  JAVA_OPTS="${JAVA_OPTS} -Dhudson.remoting.ClassFilter=${CLASS_FILTERS} -DJENKINS_HOME=${JENKINS_HOME} -Dorg.apache.tomcat.util.buf.UDecoder.ALLOW_ENCODED_SLASH=true -Dhudson.PluginManager.className=com.bswtechconsulting.jenkins.ReadOnlyPluginManager" exec /usr/libexec/tomcat/server start
fi
